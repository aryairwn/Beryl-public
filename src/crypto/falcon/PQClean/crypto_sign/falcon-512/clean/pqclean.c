...
/*
 * Deterministic Falcon-512 keypair generation.
 *
 * This is the same Falcon-512 key generation and encoding path
 * as crypto_sign_keypair(), except the 48-byte initial seed is
 * supplied by the caller instead of randombytes().
 */
int
    ...
    uint8_t *pk, uint8_t *sk, const uint8_t *seed)
{
    ...
    PQCLEAN_FALCON512_CLEAN_keygen(
        &rng, f, g, F, NULL, h, 9, tmp.b);

    inner_shake256_ctx_release(&rng);

    /* Encode private key exactly like the standard API. */
    sk[0] = 0x50 + 9;
    u = 1;
    ...
    /* Encode public key exactly like the standard API. */
    pk[0] = 0x00 + 9;
